...
function Avatar({ name }: { name: string }) {
  ...
  return (
    <div className="w-8 h-8 rounded-full text-xs font-semibold flex items-center justify-center flex-shrink-0"
      style={{ backgroundColor: '#EDD0C0', color: '#5A3828' }}>
      {initials}
    </div>
  );
}
...
function CloseBtn({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} style={{ color: '#896B5E' }}>
    ...
    <>
      ...
      {showAdd && (
        <Overlay onClose={() => setShowAdd(false)}>
          <div className="w-full max-w-md rounded-2xl shadow-2xl p-6" style={{ backgroundColor: '#FAF5F0' }}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-semibold" style={{ fontFamily: 'var(--font-serif)', color: '#2D1912' }}>
                Add Customer
              </h2>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={key}>
                  <label className="block text-xs font-medium mb-1" style={{ color: '#5A3828' }}>{label}</label>
      ...
      {viewing && (
        <Overlay onClose={() => setViewing(null)}>
          <div className="w-full max-w-sm rounded-2xl shadow-2xl p-6" style={{ backgroundColor: '#FAF5F0' }}>
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <Avatar name={viewing.company} />
                <div>
                  <div className="font-semibold" style={{ fontFamily: 'var(--font-serif)', color: '#2D1912' }}>
                    {viewing.company}
                  </div>
                  <div className="text-xs" style={{ color: '#896B5E' }}>{viewing.id.slice(0, 8)}…</div>
                </div>
              </div>
            ...
            <div className="space-y-0 mt-4">
              {[
                ...
                <div key={label} className="flex justify-between py-2.5" style={{ borderBottom: '1px solid #F5ECE4' }}>
                  <span className="text-xs font-medium" style={{ color: '#896B5E' }}>{label}</span>
                  <span className="text-sm" style={{ color: '#2D1912' }}>{value}</span>
                </div>
              ))}
            </div>
          ...
          {loading ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody style={{ backgroundColor: '#fff' }}>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-5 py-12 text-center text-sm" style={{ color: '#896B5E' }}>
                      {customers.length === 0
                  ...
                  <tr key={c.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #F5ECE4' : 'none' }}>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <Avatar name={c.company} />
                        <div>
                          <div className="font-medium" style={{ color: '#2D1912' }}>{c.company}</div>
                          <div className="text-xs" style={{ color: '#896B5E' }}>{c.id.slice(0, 8)}…</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4" style={{ color: '#5A3828' }}>{c.contact}</td>
                    <td className="px-5 py-4" style={{ color: '#896B5E' }}>{c.email}</td>
                    <td className="px-5 py-4" style={{ color: '#896B5E' }}>{c.phone || '—'}</td>
                    <td className="px-5 py-4 text-xs" style={{ color: '#896B5E' }}>
                      {new Date(c.created_at).toLocaleDateString('en-MY', { day: 'numeric', month: 'short', year: 'numeric' })}
                    </td>
                    <td className="px-5 py-4 text-right">
                      <button onClick={() => setViewing(c)} className="text-xs font-medium" style={{ color: '#C8705A' }}>
                        View →
                      </button>
                    </td>
                  </tr>
