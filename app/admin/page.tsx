...
export default async function Dashboard() {
  ...
  const statCards = [
    { label: 'Quotes This Month', value: stats.total,    sub: monthLabel,     accent: '#2D1912' },
    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {statCards.map(({ label, value, sub, accent }) => (
          <div key={label} className="rounded-xl p-5" style={{ backgroundColor: '#fff', border: '1px solid #EDD0C0' }}>
            <div className="text-3xl font-bold" style={{ color: accent }}>{value}</div>
            <div className="text-sm font-medium mt-1" style={{ color: '#2D1912' }}>{label}</div>
            <div className="text-xs mt-0.5" style={{ color: '#896B5E' }}>{sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold" style={{ color: '#2D1912' }}>Recent Quotes</h2>
          <Link href="/admin/quotes" className="text-xs font-medium" style={{ color: '#C8705A' }}>
            View all →
          </Link>
        </div>
        ...
        <div className="rounded-xl overflow-hidden" style={{ border: '1px solid #EDD0C0' }}>
          {recentQuotes.length === 0 ? (
            <div className="py-12 text-center text-sm" style={{ backgroundColor: '#fff', color: '#896B5E' }}>
              No quotes yet. <Link href="/admin/create-quote" style={{ color: '#C8705A' }}>Create your first quote →</Link>
            </div>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody style={{ backgroundColor: '#fff' }}>
                {recentQuotes.map((q, i) => {
                  ...
                  return (
                    <tr key={q.id} style={{ borderBottom: i < recentQuotes.length - 1 ? '1px solid #F5ECE4' : 'none' }}>
                      <td className="px-5 py-3.5 font-mono text-xs" style={{ color: '#A8522E' }}>{q.quote_number}</td>
                      <td className="px-5 py-3.5 font-medium" style={{ color: '#2D1912' }}>
                        {q.customers?.company ?? '—'}
                      </td>
